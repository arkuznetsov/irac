...
Процедура ЯВызываюФункциюСозданияИнформационнойБазыСИменемВКластере(Знач ИмяБазы, Знач ИмяКластера) Экспорт
	
	АгентКластера = Новый УправлениеКластером1С(, ИмяКластера);

	Кластеры = АгентКластера.Кластеры();

	Кластер = Кластеры.Получить(ИмяКластера);

	ПараметрыИБ = Новый Структура();

	ПараметрыИБ.Вставить("ТипСУБД"                                , Перечисления.ТипыСУБД.MSSQLServer);
	ПараметрыИБ.Вставить("АдресСервераСУБД"                        , ИмяКластера);
