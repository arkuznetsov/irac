...
Перем Кластер_Агент;       // - УправлениеКластером1С    - объект агента кластера
...
Функция Вывод_АгентВерсия()
	...
	ПараметрыОбъекта = Новый КомандыОбъекта(Кластер_Агент,
	                                        Перечисления.РежимыАдминистрирования.Агенты,
	                                        ПараметрыКоманды);

	ВыводКоманды = Параметры().Агент_Версия;

	СтрокаПараметров = ПараметрыОбъекта.ПараметрыКомандыСтрокой("Версия");

	Возврат ОписаниеВыводаКоманды(СтрокаПараметров, ВыводКоманды);


КонецФункции // Агент_Версия()
...
Функция Вывод_АдминистраторыСписок()
	...
	СтрокаПараметров = ПараметрыОбъекта.ПараметрыКомандыСтрокой("Список");
...
Функция Вывод_КластерыПараметры()
	...
	Для Каждого ТекКластер Из Кластеры Цикл
		...
		СтрокаПараметров = ПараметрыОбъекта.ПараметрыКомандыСтрокой("Описание");

		Результат.Добавить(ОписаниеВыводаКоманды(СтрокаПараметров, ВыводКоманды));
...
Функция Вывод_КластерыПараметрыПослеИзменения()
	
	Результат = Новый Массив();

	Кластеры = РазобратьВыводКоманды(ВозвращаемыеЗначения["Кластеры.Изменение"]);
...
КонецФункции // Вывод_КластерыПараметрыПослеИзменения()
...
Функция Вывод_СерверыДобавить()
	...
	СтрокаПараметров = ПараметрыОбъекта.ПараметрыКомандыСтрокой("Добавить");
...
Функция Вывод_ИБПолныеПараметры()
	...
	Для Каждого ТекИб Из ИБ Цикл
		...
		СтрокаПараметров = ПараметрыОбъекта.ПараметрыКомандыСтрокой("ПолноеОписание");
...
Функция Вывод_СчетчикиРесурсовПараметрыЗначения()
	...
	СтрокаПараметров = ПараметрыОбъекта.ПараметрыКомандыСтрокой("Значения");
...
Процедура Инициализация()
	...
	ПараметрыКластера.Вставить("Версия", Вывод_АгентВерсия());
	...
	ПараметрыКластера.Вставить("Кластеры",
	    Новый Структура("Список, Параметры, Изменение, Администраторы, Добавить, Изменить, Удалить"));
	ПараметрыКластера.Кластеры.Список            = Вывод_КластерыСписок();
	ПараметрыКластера.Кластеры.Параметры         = Вывод_КластерыПараметры();
	ПараметрыКластера.Кластеры.Изменение         = Вывод_КластерыПараметрыПослеИзменения();
...
Функция Параметры(Обновить = Ложь) Экспорт
	...
	ЭтоСерверСборок = ВРег(ПолучитьПеременнуюСреды("CI")) = "TRUE";
