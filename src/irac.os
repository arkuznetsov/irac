...
Процедура ОписаниеСерверов(АгентКластера, Кластер)
	
	Серверы = Новый СерверыКластера(АгентКластера, Кластер);
...
Функция ПолучитьСписокБаз(НомерКластера = 0) Экспорт
	...
	Сообщить(Агент.ПараметрыАгента());
	...
	Для Каждого ТекЭлемент Из Кластеры.ПолучитьСписок() Цикл
		...
		ОписаниеСерверов(Агент, ТекЭлемент.Значение);
