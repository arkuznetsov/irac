...
//
Процедура ПриСозданииОбъекта(Сервер = "localhost"
						   , Порт = 1545
						   , Версия = "8.3"
						   , Администратор = ""
...
Функция СтрокаПодключения() Экспорт
	...
	Лог.Отладка("Сервер " + АдресСервера);
	Лог.Отладка("Порт <" + ПортСервера + ">");

	Сервер = "";
	Если Не ПустаяСтрока(АдресСервера) Тогда
		Сервер = АдресСервера;
		Если Не ПустаяСтрока(ПортСервера) Тогда
			Сервер = Сервер + ":" + ПортСервера;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеПодключения() Экспорт
	...
	Возврат СокрЛП(АдресСервера) + ":" + Формат(ПортСервера, "ЧГ=") + "(v." + СокрЛП(Версия1С) + ")";
