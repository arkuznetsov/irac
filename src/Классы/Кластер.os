...
Процедура ПриСозданииОбъекта(АгентКластера, Ид, Администратор = "", ПарольАдминистратора = "")
	...
	Кластер_Администраторы = Неопределено;
	Кластер_Серверы = Неопределено;
	Кластер_Менеджеры = Неопределено;
	Кластер_ИБ = Неопределено;
	Кластер_Сеансы = Неопределено;
...
//
Функция Параметры(ОбновитьПринудительно = Ложь) Экспорт
...
//   
// Параметры:
//   ИмяПоля			 	- Строка		- Имя поля описания кластера
//   ОбновитьПринудительно 	- Булево		- Истина - обновить список (вызов RAC)
//
// Возвращаемое значение:
//	Строка - параметры кластера 1С
...
//
Функция Администраторы(ОбновитьПринудительно = Ложь) Экспорт

	Если Служебный.ТребуетсяОбновление(Кластер_Администраторы,
			МоментАктуальности, ПериодОбновления, ОбновитьПринудительно) Тогда
...
//
Функция Серверы(ОбновитьПринудительно = Ложь) Экспорт
	
	Если Служебный.ТребуетсяОбновление(Кластер_Серверы,
			МоментАктуальности, ПериодОбновления, ОбновитьПринудительно) Тогда
...
//
Функция Менеджеры(ОбновитьПринудительно = Ложь) Экспорт
	
	Если Служебный.ТребуетсяОбновление(Кластер_Менеджеры,
			МоментАктуальности, ПериодОбновления, ОбновитьПринудительно) Тогда
...
//
Функция ИнформационныеБазы(ОбновитьПринудительно = Ложь) Экспорт
	
	Если Служебный.ТребуетсяОбновление(Кластер_ИБ,
			МоментАктуальности, ПериодОбновления, ОбновитьПринудительно) Тогда
...
//
// Возвращаемое значение:
//	Сеансы - список информационных баз 1С
//
Функция Сеансы(ОбновитьПринудительно = Ложь) Экспорт
	
	Если Служебный.ТребуетсяОбновление(Кластер_Сеансы,
			МоментАктуальности, ПериодОбновления, ОбновитьПринудительно) Тогда
