...
//   
// Параметры:
//   Кластер  - Строка,Число,Соотвествие - * идентификатор кластера
//                                         * адрес кластера в виде <сервер>:<порт>
//                                         * номер порта кластера
//                                         * произвольный отбор
//   РежимОбновления         - Число     - 1 - обновить данные принудительно (вызов RAC)
//                                         0 - обновить данные только по таймеру
//                                        -1 - не обновлять данные
...
Функция Получить(Знач Кластер, Знач РежимОбновления = 0, КакСоответствие = Ложь) Экспорт
	...
	ИначеЕсли ТипЗнч(Кластер) = Тип("Строка") Тогда
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(Кластер) = Тип("Число") Тогда
		Отбор.Вставить("port", Кластер);
	Иначе
		Для каждого КЗ Из Кластер Цикл
			Отбор.Вставить(КЗ.Ключ, КЗ.Значение);
		КонецЦикла;
	КонецЕсли;
