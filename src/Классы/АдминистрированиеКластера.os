...
Процедура ПриСозданииОбъекта(АдресСервера
	...
	Агент_ИсполнительКоманд = Новый ИсполнительКоманд(Версия);
...
// Процедура устанавливает исполнитель команд
//   
// Параметры:
...
//
Функция ИсполнительКоманд(Знач НовыйИсполнитель = Неопределено) Экспорт

	ТекИсполнитель = Агент_ИсполнительКоманд;
...
Функция ОписаниеПодключения() Экспорт
	...
	Возврат СокрЛП(Админ_АдресСервера) + ":" + Формат(Админ_ПортСервера, "ЧГ=") +
			" (v." + СокрЛП(Агент_ИсполнительКоманд.ВерсияПлатформы()) + ")";
