...
Процедура ПриСозданииОбъекта(ВерсияИлиПутьКРАК = "8.3")
	...
	ШаблонПроверки = "8.";
	Если Лев(ВерсияИлиПутьКРАК, СтрДлина(ШаблонПроверки)) = ШаблонПроверки Тогда
...
// Функция получает путь к утилите RAC
//   
// Возвращаемое значение:
	...
	Для Каждого ВозможныйПуть Из КаталогиУстановкиПлатформы Цикл
		...
		Для каждого Подкаталог1С Из МассивФайлов Цикл
			...
			Если НЕ ФайлУтилитыАдминистрирования.Существует() Тогда
				...
				Продолжить;
			КонецЕсли;	 
			ОписаниеКаталога = Новый Структура("Версия, ФайлУтилитыАдминистрирования", Подкаталог1С.Имя, ФайлУтилитыАдминистрирования);
