...
//
Процедура ПриСозданииОбъекта(Версия = "8.3")
	...
	ИспользоватьВерсиюПлатформы(Версия);
	ПутьКПлатформе1С = ПутьКПлатформе1С();
	ВерсияПлатформы = ПолучитьВерсиюПлатформы(ПутьКПлатформе1С);
...
Функция ПолучитьПутьКВерсииПлатформыLinux(Знач Версия)
	...
	Если ФайлРАК.Существует() Тогда
		...
		Исключение
			...
			|" + ОписаниеОшибки(), Версия, СтрокаЗапуска);
		КонецПопытки;
	КонецЕсли;
...
//	Строка - путь к версии платформы 1С
//
Функция ПолучитьПутьКВерсииПлатформы(Знач Версия) Экспорт
	
	ПроверяемаяВерсия = "8.";
	Если Лев(Версия, СтрДлина(ПроверяемаяВерсия)) <> ПроверяемаяВерсия Тогда
...
//
Функция ПолучитьВерсиюПлатформы(Знач Путь)
...
// Функция возвращает версию платформы 1С
//   
// Возвращаемое значение:
//	Строка - старая (текущая) версия платформы 1С
//
...
Функция ЗапуститьИПодождать(Знач Параметры)
	...
	Приложение = Служебный.ОбернутьВКавычки(ПутьКПлатформе1С());
