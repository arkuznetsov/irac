...
// Функция возвращает версию агента кластера
//
// Возвращаемое значение:
//    Строка - версия агента кластера
//
Функция Версия() Экспорт

	ПараметрыКоманды = Новый Соответствие();
	ПараметрыКоманды.Вставить("СтрокаПодключенияАгента", СтрокаПодключения());
	ПараметрыКоманды.Вставить("ПараметрыАвторизацииАгента", ПараметрыАвторизации());
	
	ПараметрыОбъекта.УстановитьЗначенияПараметровКоманд(ПараметрыКоманды);

	КодВозврата = ПараметрыОбъекта.ВыполнитьКоманду("Версия");

	Если НЕ КодВозврата = 0 Тогда
		ВыводКоманды = ВыводКоманды(Ложь);
		ТекстОшибки = СтрШаблон("Ошибка получения версии агента ""%1"", КодВозврата = %2:%3%4",
		                        СтрокаПодключения(),
		                        КодВозврата,
		                        Символы.ПС,
		                        ВыводКоманды);
