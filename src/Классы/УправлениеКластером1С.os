...
Перем ПараметрыОбъекта;          // параметры этого объекта управления кластерами
...
//
Процедура ДобавитьПараметрыСУБДИБ(ИБ_Ид, ТипСУБД, Сервер, Пользователь, Пароль) Экспорт

	Если НЕ ТипЗнч(ИБ_ПараметрыСУБД) = Тип("Соответствие") Тогда
		ИБ_ПараметрыСУБД = Новый Соответствие();
	КонецЕсли;

	ИБ_ПараметрыСУБД.Вставить(ИБ_Ид, Новый Структура("ТипСУБД, Сервер, Пользователь, Пароль", ТипСУБД, Сервер, Пользователь, Пароль));
