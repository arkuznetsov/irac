...
Процедура ЗаполнитьОписанияПараметровОбъекта(Знач ИмяОбъекта)
	
	ТипыОбъектовКластера = Служебный.ПрочитатьДанныеИзМакетаJSON("ТипыОбъектовКластера");

	МассивИмен = СтрРазделить(ИмяОбъекта, ".");

	Свойства = ТипыОбъектовКластера;

	Для Каждого ТекИмя Из МассивИмен Цикл
		Если НЕ Свойства[ТекИмя].Свойство("РежимАдминистрирования") Тогда
			Возврат;
		КонецЕсли;
		Если НЕ Свойства[ТекИмя].Свойство("Свойства") Тогда
			Возврат;
		КонецЕсли;
		Свойства = Свойства[ТекИмя].Свойства;
	КонецЦикла;

	Для Каждого ТекСвойство Из Свойства Цикл
		Если НЕ ТекСвойство.Значение.Свойство("ИмяРАК") Тогда
			Продолжить;
		КонецЕсли;
		ДобавитьОписаниеСвойстваОбъекта(ТекСвойство.Ключ,
										ТекСвойство.Значение.ИмяРАК,
										?(ТекСвойство.Значение.Свойство("ПоУмолчанию"), ТекСвойство.Значение.ПоУмолчанию, ""),
										?(ТекСвойство.Значение.Свойство("Использование"), ТекСвойство.Значение.Использование, "Чтение"));
	КонецЦикла;
...
Процедура Параметры_Кластер_Свойства() Экспорт
	...
	ЗаполнитьОписанияПараметровОбъекта("Кластеры");
...
// Процедура заполняет массив описаний параметров счетчиков потребления ресурсов
//
...
// Процедура заполняет массив описаний параметров значений счетчика потребления ресурсов
//
Процедура Параметры_СчетчикРесурсов_Значения_Свойства() Экспорт

	ДобавитьОписаниеСвойстваОбъекта("Отбор"                          , "object");
...
КонецПроцедуры // Параметры_СчетчикРесурсов_Значения_Свойства()
...
//   
Процедура ДобавитьОписаниеСвойстваОбъекта(Знач Имя
	                                    , Знач ИмяРАК
	                                    , Знач ПоУмолчанию = ""
	...
	ОписаниеСвойства.Вставить("ИмяРАК"          , ИмяРАК);
	ОписаниеСвойства.Вставить("ПараметрКоманды" , "");
	ОписаниеСвойства.Вставить("ПоУмолчанию"     , ПоУмолчанию);
