...
Процедура ДобавитьПараметрКоманды(Знач Параметр)
	...
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		...
		Если Параметр.Свойство("Авторизация") Тогда
			ДобавитьПараметрыАвторизации(Параметр.Авторизация, Параметр.Параметр);
...
//                                                         или не заполнено будет выдано исключение
//
Процедура ДобавитьПараметрыАвторизации(Знач ТипОбъектаАвторизации, Знач Имя, Обязательный = Ложь)
	...
	ИдЗначения = Строка(Новый УникальныйИдентификатор());

	КэшПараметровАвторизации.Вставить(СтрШаблон("%1_user", ИдЗначения), ПараметрыАвторизации.Администратор);

	ПараметрыЗапуска.Добавить(СтрШаблон("--%1-user=%2",
	                                    ПараметрыАвторизации.Тип,
	                                    СтрШаблон("%1_user", ИдЗначения)));

	Если НЕ ПустаяСтрока(ПараметрыАвторизации.Пароль) Тогда
	    КэшПараметровАвторизации.Вставить(СтрШаблон("%1_pwd", ИдЗначения), ПараметрыАвторизации.Пароль);

	    ПараметрыЗапуска.Добавить(СтрШаблон("--%1-pwd=%2",
	                                        ПараметрыАвторизации.Тип,
	                                        СтрШаблон("%1_pwd", ИдЗначения)));
	КонецЕсли;
