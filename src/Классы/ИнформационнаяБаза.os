...
Функция ПолучитьПолноеОписаниеИБ()
	...
	КодВозврата = Кластер_Агент.ВыполнитьКоманду(ПараметрыЗапуска.Получить());
...
Функция ПолучитьОписаниеИБ()
	...
	Кластер_Агент.ВыполнитьКоманду(ПараметрыЗапуска.Получить());
...
Функция СтрокаАвторизации() Экспорт
	...
	СтрокаАвторизации = "";
	Если Не ПустаяСтрока(ПараметрыАдминистратора.Администратор) Тогда
		СтрокаАвторизации = СтрШаблон("--infobase-user=%1 --infobase-pwd=%2",
		                              ПараметрыАдминистратора.Администратор,
		                              ПараметрыАдминистратора.Пароль);
	КонецЕсли;
