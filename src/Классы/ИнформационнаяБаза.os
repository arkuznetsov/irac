...
Процедура ОбновитьДанные(ОбновитьПринудительно = Ложь) Экспорт
	...
	ИБ_Параметры = Новый Соответствие();
...
Функция ПолучитьОписаниеИБ()
	...
	КодВозврата = Кластер_Агент.ВыполнитьКоманду(ПараметрыЗапуска, Ложь);
	
	Если НЕ КодВозврата = 0 Тогда
		Если Найти(Кластер_Агент.ВыводКоманды(), "Недостаточно прав пользователя") = 0 Тогда
			ВызватьИсключение Кластер_Агент.ВыводКоманды();
		Иначе
