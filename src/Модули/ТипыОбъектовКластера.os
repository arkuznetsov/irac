...
//       *Основное           - Булево            - Истина - основное свойство типа объектов
...
Функция СвойстваОбъекта(Знач ТипОбъектов, ТолькоОсновные = Ложь) Экспорт
	...
	Если ТолькоОсновные Тогда
		...
		Для Каждого ТекСвойство Из ВсеСвойства Цикл
			Если ТекСвойство.Основное Тогда
				Свойства.Добавить(ТекСвойство);
			КонецЕсли;
		КонецЦикла;
	Иначе
		Свойства = ВсеСвойства;
	КонецЕсли;
...
Функция ПолучитьСтруктуруОписанияСвойства()
	...
	ОписаниеСвойства.Вставить("Основное"       , Тип("Булево"));
	ОписаниеСвойства.Вставить("Тип"            , Тип("Строка"));
...
//   Имя                - Строка                - имя свойства объекта
//   ИмяРАК             - Строка                - имя свойства, как оно возвращается утилитой RAC
//   Основное           - Булево                - признак основного свойства объекта (для ИБ и соединений)
//   Тип                - Тип                   - тип свойства объекта
//   ПоУмолчанию        - Произвольный          - значение свойства объекта по умолчанию
...
Функция ПолучитьОписаниеСвойства(Знач Имя
	...
	Если ВРег(Лев(ПоУмолчанию, 14)) = "$ПЕРЕЧИСЛЕНИЯ." Тогда
	...
	ОписаниеСвойства.Основное     = Основное;
	ОписаниеСвойства.Тип          = Тип;
...
//       *ИмяРАК                 - Строка                - имя свойства, как оно возвращается утилитой RAC
//       *Основное               - Строка                - флаг основного свойства объекта (для ИБ и соединений)
//       *ПоУмолчанию            - Произвольный          - значение свойства объекта по умолчанию
//       *Использование          - Строка, Структура     - строка содержащая флаги использования, разделенные ","
//                                                         (Чтение, Добавление, Изменение)
//                                                         если указана структура, то в значении можно переопределить
...
Процедура ДобавитьОписаниеСвойстваОбъекта(ОписаниеСвойствОбъекта, Знач Имя, Знач СтруктураСвойства)
	...
	Если СтруктураСвойства.Свойство("Тип") Тогда
		СтруктураСвойства.Тип = Тип(СтруктураСвойства.Тип);
	Иначе
		СтруктураСвойства.Вставить("Тип", ТипЗнч(СтруктураСвойства.ПоУмолчанию));
	КонецЕсли;
	...
	ОписаниеСвойства = ПолучитьОписаниеСвойства(Имя, 
	                                            ...
	                                            СтруктураСвойства.Основное,
	                                            СтруктураСвойства.Тип,
...
//       *ПериодОбновления           - Число                 - период обновления данных объекта в миллисекундах
...
Процедура ДобавитьСвойстваТипаОбъектов(Знач ОписаниеТипаОбъектов
	...
	ЕстьФлагОсновное = Ложь;
	Для Каждого ТекСвойство Из ОписаниеТипаОбъектов.Свойства Цикл
		Если ТекСвойство.Значение.Свойство("Основное") Тогда
			ЕстьФлагОсновное = Истина;
		КонецЕсли;
	КонецЦикла;

	Для Каждого ТекСвойство Из ОписаниеТипаОбъектов.Свойства Цикл

		Если НЕ ТекСвойство.Значение.Свойство("Основное") Тогда
			ТекСвойство.Значение.Вставить("Основное", НЕ ЕстьФлагОсновное);
		КонецЕсли;
...
Процедура ДобавитьОписаниеТипаОбъектов(Знач ОписаниеТипаОбъектов
	...
	Если ОписаниеТипаОбъектов.Свойство("ПериодОбновления") Тогда
		ОписаниеТипа.Вставить("ПериодОбновления"      , ОписаниеТипаОбъектов.ПериодОбновления);
	КонецЕсли;
	Если ТипЗнч(ИменаРодителя) = Тип("Массив") И ЗначениеЗаполнено(ИменаРодителя) Тогда
...
//
// Возвращаемое значение:
//   Массив из Строка        - возможные комбинации имен родителя и типа
//
Функция ПолучитьВозможныеИменаТипа(ИменаТипа, ИменаРодителя = Неопределено)

	Если НЕ (ТипЗнч(ИменаРодителя) = Тип("Массив") И ЗначениеЗаполнено(ИменаРодителя)) Тогда
		Возврат ИменаТипа;
	КонецЕсли;
