...
Функция ЗапуститьИПодождать(Знач Параметры)
	...
	Для Каждого Параметр Из Параметры Цикл
		...
		Если НЕ (Лев(Параметр, 11) = "--agent-pwd"
			 ИЛИ Лев(Параметр, 13) = "--cluster-pwd"
			 ИЛИ Лев(Параметр, 14) = "--infobase-pwd"
			 ИЛИ Лев(Параметр, 5) = "--pwd") Тогда
	...
	мВыводКоманды = Команда.ПолучитьВывод();
...
Функция СтруктураПараметровАдминистратора() Экспорт

	Возврат Новый Структура("Имя,
							|Пароль,
							|Описание,
							|СпособыАвторизации,
							|ПользовательОС");
